import {
    Button, Modal, Form, Input, Radio, Row, Col, Select, Popover, Icon, Checkbox, Table, Popconfirm, InputNumber, Card, message, 
} from 'antd';
...
import { ddnsRecordUpdate } from '~/api/service';
...
class RecordForm extends React.Component {
    ...
    handleSubmit = (e) => {
        ...
        let values = {
            ...
            plugin: this.state.pluginValue,
            name: this.state.nameValue,
            value: this.state.recordValue,
        };
        console.log(values);
        ddnsRecordUpdate(values).then(res => {
            if (!res.success) {
                message.error(res.message);

            } else {
                message.info(res.message);
                this.props.history.push('/ddns/record/index');
            }
        });
    }
    handleAddRow = () => {
                                ...
                                <div style={{ marginBottom: 16 }}>
                                    <Form onSubmit={this.handleSubmit}>
                                        ...
                                        <Form.Item {...tailFormItemLayout}>
                                            <Button type="primary" htmlType="submit" size="large">提交</Button><Link to="/ddns/record/index"><Button size="large">返回</Button></Link>
                                        </Form.Item>
                                    </Form>
                                </div>
                            </Card>
